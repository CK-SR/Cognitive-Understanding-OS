...
class SQLiteStore:
    ...
    def list_due_tasks(
        self, due_date: str, paper_id: str | None = None
    ) -> list[ReviewTask]:
        conn = sqlite3.connect(self.db_path)
        if paper_id:
            rows = conn.execute(
            ...
            rows = conn.execute(
                "select task_id,paper_id,question,due_date,status from review_tasks where due_date<=? and status='pending'",
                (due_date,),
            ).fetchall()
        conn.close()
        return [
            ReviewTask(
                task_id=r[0], paper_id=r[1], question=r[2], due_date=r[3], status=r[4]
            )
            for r in rows
        ]
    ...
    def update_review_task_status(self, task_id: str, status: str) -> None:
        conn = sqlite3.connect(self.db_path)
        conn.execute(
            "update review_tasks set status=? where task_id=?", (status, task_id)
        )
        conn.commit()
        conn.close()
