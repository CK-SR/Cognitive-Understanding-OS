...
def _load_parsed_document(paper_dir: Path) -> ParsedDocument:
    return ParsedDocument.model_validate_json((paper_dir / "parsed_document.json").read_text(encoding="utf-8"))
...
def run_map(paper_dir: Path, llm, prompt_dir: Path = Path("prompts"), max_input_chars: int = 12000) -> None:
    ...
    problem_prompt = registry.load("problem_model.md", {"document_markdown": compact_markdown})
    ...
    graph_prompt = registry.load(
        "graph_builder.md",
        {"document_markdown": compact_markdown, "problem_model_json": problem.model_dump_json(indent=2)},
    )
    ...
    question_prompt = registry.load("socratic_question.md", {"candidate_graph_json": graph.model_dump_json(indent=2)})
    questions = llm.chat_json([{"role": "user", "content": question_prompt}], QuestionSet)
    (cognitive_dir / "key_questions.json").write_text(questions.model_dump_json(indent=2), encoding="utf-8")
    (cognitive_dir / "key_questions.md").write_text("\n".join(f"- {q.question}" for q in questions.questions), encoding="utf-8")

    reading_map = ReadingMap(steps=["Read problem statement", "Read mechanism", "Read evidence & limitations"])
    (cognitive_dir / "reading_map.md").write_text("\n".join(f"{i+1}. {s}" for i, s in enumerate(reading_map.steps)), encoding="utf-8")
    (cognitive_dir / "graph_summary.md").write_text(export_graph_summary_md(graph), encoding="utf-8")
    (cognitive_dir / "map_trace.json").write_text(
        json.dumps({"doc_id": parsed.doc_id, "nodes": len(graph.nodes), "edges": len(graph.edges)}, indent=2),
        encoding="utf-8",
    )
