...
def run_audit(
    ...
    graph = json.loads(candidate_graph_path.read_text(encoding="utf-8"))
    ...
    for turn in session.turns:
        ...
        prompt = prompt.replace(
            "{{candidate_graph_context}}", json.dumps(graph, ensure_ascii=False)
        )
    ...
    (cognitive_dir / "review_tasks.json").write_text(
        json.dumps(
            [task.model_dump() for task in build_review_tasks(paper_dir.name)],
            ensure_ascii=False,
            indent=2,
        ),
        encoding="utf-8",
